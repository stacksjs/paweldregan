  ...
  sites: {
    ...
    api: {
      ...
      preStart: [
        ...
        'for f in $(find database/migrations -maxdepth 1 -name "*.sql" ! -name "*.down.sql" | sort); do PGPASSWORD="$DB_PASSWORD" psql -v ON_ERROR_STOP=1 -h 127.0.0.1 -U paweldregan -d paweldregan -f "$f"; done',
      ],
      env: {
        NODE_ENV: 'production',
        APP_ENV: 'production',
